...
class Solution:
    def ladderLength(self, beginWord: str, endWord: str, wordList: [str]) -> int:
        ...
        headCheckStack = [beginWord]
        footCheckStack = [endWord]
        ...
        while len(headCheckStack) > 0 and len(footCheckStack) > 0:
            headWord = headCheckStack.pop(0)
            footWord = footCheckStack.pop(0)

            for idx in range(len(headWord)):
                for k in range(26):
                    tempHead = headWord[:idx] + chr(ord('a') + k) + headWord[idx + 1:]
                    tempFoot = footWord[:idx] + chr(ord('a') + k) + footWord[idx + 1:]
                    if tempHead in wordSet and headDic.get(tempHead) is None:
                        headDic[tempHead] = headDic[headWord] + 1
                        tempHeadStack.append(tempHead)
                    if tempFoot in wordSet and footDic.get(tempFoot) is None:
                        footDic[tempFoot] = footDic[footWord] + 1
                        tempFootStack.append(tempFoot)

            if headDic.get(footWord) is not None:
                if res == 0:
                    res = headDic[footWord] + footDic[footWord] - 1
                else:
                    res = min(res, headDic[footWord] + footDic[footWord] - 1)
            if footDic.get(headWord) is not None:
                if res == 0:
                    res = headDic[headWord] + footDic[headWord] - 1
                else:
                    res = min(res, headDic[headWord] + footDic[headWord] - 1)

            if len(headCheckStack) == 0:
                headCheckStack = tempHeadStack[:]
                tempHeadStack.clear()
                if res > 0:
                    break
            if len(footCheckStack) == 0:
                footCheckStack = tempFootStack[:]
                tempFootStack.clear()
                if res > 0:
                    break
...
# print(Solution().ladderLength("hto", "cog", ["hot", "dot", "dog", "lot", "log"]))  # 0
# print(Solution().ladderLength("a", "c", ["a", "b", "c"]))  # 2
# print(Solution().ladderLength("leet", "code", ["lest", "leet", "lose", "code", "lode", "robe", "lost"]))  # 6
# print(Solution().ladderLength("lost", "cost", ["most", "fist", "lost", "cost", "fish"]))  # 2
# print(Solution().ladderLength("sail", "ruip",
#                               ["rain", "ruin", "gain", "ruip", "grin", "grit", "sail", "main", "pain", "pair", "pail",
#                                "mail"]))  # 6
t1 = time.time()

print(Solution().ladderLength("nanny",
                              "aloud",

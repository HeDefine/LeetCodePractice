        ...
        for x, line in enumerate(board):
            for y, val in enumerate(line):
                ...
                for direction in directions:
                    if 0 <= x + direction[0] < len(board) and 0 <= y + direction[1] < len(line):
                        count += 1 if abs(board[x + direction[0]][y + direction[1]]) == 1 else 0
                if val == 0 and count == 3:
                    board[x][y] = 2
                if val == 1 and (3 < count or count < 2):
                    board[x][y] = -1

        for x, line in enumerate(board):
            for y, val in enumerate(line):
                board[x][y] = 1 if val > 0 else 0

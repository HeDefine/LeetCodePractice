...
class Solution:
    def isAlienSorted(self, words: [str], order: str) -> bool:
        ...
        for word in words:
            temp = ''
            for ch in word:
                temp += chr(ord('a') + orderDic[ch])
            wordOrder.append(temp)
        return sorted(wordOrder) == wordOrder
...
print(Solution().isAlienSorted(words=["apple", "app"], order="abcdefghijklmnopqrstuvwxyz"))  # False

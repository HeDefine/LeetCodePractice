...
class Solution:
    def countBalls(self, lowLimit: int, highLimit: int) -> int:
        ...
        for num in range(lowLimit, highLimit+1):
            sum_val = sum([int(i) for i in list(str(num))])
            res_dict[sum_val] = res_dict.get(sum_val, 0) + 1
            res = max(res, res_dict[sum_val])
        return res
        
print(Solution().countBalls(lowLimit = 1, highLimit = 10))  # 2
print(Solution().countBalls(lowLimit = 5, highLimit = 15))  # 2
print(Solution().countBalls(lowLimit = 19, highLimit = 28)) # 2
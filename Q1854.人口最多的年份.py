...
class Solution:
    def maximumPopulation(self, logs: list) -> int:
        res = dict()
        for line in logs:
            for year in range(line[0], line[1]):
                res[year] = res.get(year, 0) + 1
        return sorted(res.items(), key=lambda i: (i[1], -i[0]), reverse=True)[0][0]

print(Solution().maximumPopulation(logs = [[1993,1999],[2000,2010]]))   # 1993
print(Solution().maximumPopulation(logs = [[1950,1961],[1960,1971],[1970,1981]]))   # 1960